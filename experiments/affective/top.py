...
work_dir = locate_dirs( get_path('affective')
	                  , ['results', 'no-data'] )
...
############################################################
'''
	Load affective norm pairs
'''
norms = ['arousal', 'concreteness', 'dominance', 'happiness']

paths = [(n, os.path.join(get_path('affective-data'), n + '.txt')) \
        for n in norms]

NORM = { n : open_norm(p) for n,p in paths }
...
to_path = lambda p : os.path.join( work_dir['no-data'], p + '.pkl')
